	...
	while(xMessageBufferReceive(handlePduRxMessageBuffer, &rxRawMsg, sizeof(rxRawMsg), 0) != 0) {
		...
		if(checkRxMessage<messages::PDU_RX_Control>(rxRawMsg)) {
			...
			if(!pduEnabled) {
				...
				outputState.LPCh4_frontECU.set(			manualControlMsg.get<signals::PDU_LPCh4_Enable>());
			...
			if(!pduEnabled) {
				outputState.LPCh1_opticalSensor.setDuty(				dutyMsg.get<signals::PDU_LPCh1_Dutycycle>());
				outputState.LPCh2.setDuty(				dutyMsg.get<signals::PDU_LPCh2_Dutycycle>());
				outputState.LPCh3_HVCU.setDuty(			dutyMsg.get<signals::PDU_LPCh3_Dutycycle>());
				outputState.LPCh8_telemetry.setDuty(	dutyMsg.get<signals::PDU_LPCh8_Dutycycle>());
				outputState.LPCh9_logger.setDuty(dutyMsg.get<signals::PDU_LPCh9_Dutycycle>());
				outputState.LPCh10.setDuty(				dutyMsg.get<signals::PDU_LPCh10_Dutycycle>());
			}
			...
			if(!pduEnabled) {
				outputState.HPCh1_projectXX.setDuty(		dutyMsg.get<signals::PDU_HPCh1_Dutycycle>());
				outputState.HPCh2_coolingPump.setDuty(		dutyMsg.get<signals::PDU_HPCh2_Dutycycle>());
...
void readAndSendData() {
	...
	msgDebug1.set<signals::PDU_LPCh4_ShortCnt>(outputState.LPCh4_frontECU.numShorts());
	msgDebug1.set<signals::PDU_LPCh4_State>(outputState.LPCh4_frontECU.getStatus());
...
void updateChannels() {
	...
	HAL_GPIO_WritePin(LP4_control_GPIO_Port, LP4_control_Pin, outputState.LPCh4_frontECU.getSwitch() ? GPIO_PIN_SET : GPIO_PIN_RESET);
