/* DO NOT MODIFY. THIS FILE WAS GENERATED AUTOMATICALLY BY CZ2CPP V1.7.7.
 *
 * This header file was generated from 'pod2023_gen.dbc'.
 * It contains all CANzero defines (e.g. CAN-IDs) for the node 'BrakeF'.
 *
 * Florian Keck
 * florian.keck@mu-zero.de
 * Copyright 2023, mu-zero HYPERLOOP e.V.
 */
#ifndef CANZERO_DEFINES_H
#define CANZERO_DEFINES_H

#pragma once

// Node ID and name
#define CANZERO_NODE_ID     0x11
#define CANZERO_NODE_NAME   "BrakeF"

// CAN-ID of the nodes function codes
#define CANZERO_EMCY_CAN_ID           0x091
#define CANZERO_TX_PDO1_CAN_ID        0x191
#define CANZERO_RX_PDO1_CAN_ID        0x1D1
#define CANZERO_TX_PDO2_CAN_ID        0x211
#define CANZERO_RX_PDO2_CAN_ID        0x251
#define CANZERO_TX_PDO3_CAN_ID        0x291
#define CANZERO_RX_PDO3_CAN_ID        0x2D1
#define CANZERO_TX_PDO4_CAN_ID        0x311
#define CANZERO_RX_PDO4_CAN_ID        0x351
#define CANZERO_TX_PDO5_CAN_ID        0x391
#define CANZERO_TX_PDO6_CAN_ID        0x3D1
#define CANZERO_TX_PDO7_CAN_ID        0x411
#define CANZERO_TX_PDO8_CAN_ID        0x451
#define CANZERO_TX_PDO9_CAN_ID        0x491
#define CANZERO_TX_PDO10_CAN_ID       0x4D1
#define CANZERO_TX_PDO11_CAN_ID       0x511
#define CANZERO_TX_PDO12_CAN_ID       0x551
#define CANZERO_SDO_RESP_CAN_ID       0x591
#define CANZERO_SDO_REQ_UP_CAN_ID     0x5D1
#define CANZERO_SDO_REQ_DOWN_CAN_ID   0x611
#define CANZERO_HEARTBEAT_CAN_ID      0x711
#define CANZERO_BTL_TX_CAN_ID         0x751
#define CANZERO_BTL_RX_CAN_ID         0x791

// CAN DLC of the nodes function codes
#define CANZERO_EMCY_CAN_DLC           4
#define CANZERO_TX_PDO1_CAN_DLC        3
#define CANZERO_RX_PDO1_CAN_DLC        1
#define CANZERO_TX_PDO2_CAN_DLC        0
#define CANZERO_RX_PDO2_CAN_DLC        0
#define CANZERO_TX_PDO3_CAN_DLC        0
#define CANZERO_RX_PDO3_CAN_DLC        0
#define CANZERO_TX_PDO4_CAN_DLC        0
#define CANZERO_RX_PDO4_CAN_DLC        0
#define CANZERO_TX_PDO5_CAN_DLC        0
#define CANZERO_TX_PDO6_CAN_DLC        0
#define CANZERO_TX_PDO7_CAN_DLC        6
#define CANZERO_TX_PDO8_CAN_DLC        0
#define CANZERO_TX_PDO9_CAN_DLC        0
#define CANZERO_TX_PDO10_CAN_DLC       0
#define CANZERO_TX_PDO11_CAN_DLC       0
#define CANZERO_TX_PDO12_CAN_DLC       0
#define CANZERO_SDO_RESP_CAN_DLC       8
#define CANZERO_SDO_REQ_UP_CAN_DLC     8
#define CANZERO_SDO_REQ_DOWN_CAN_DLC   2
#define CANZERO_HEARTBEAT_CAN_DLC      1
#define CANZERO_BTL_TX_CAN_DLC         8
#define CANZERO_BTL_RX_CAN_DLC         8

#endif // CANZERO_DEFINES_H